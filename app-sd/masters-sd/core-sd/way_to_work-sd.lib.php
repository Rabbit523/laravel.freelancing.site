<?php
	class WayToWork extends Home {
		public function __construct($module, $id = 0, $objPost = NULL, $searchArray = array(), $type = '') {
			global $db;
			$this->db = $db;
			$this->id = $id;
			$this->module = $module;
			$this->table = 'tbl_way_to_work';
			parent::__construct();
	...
	public function getForm() {
		...
		$replace_array = array(
			...
			"%ID%" => $this->id,
			"%OLD_IMAGE%" => $this->icon,
			"%OLD_IMAGE_SRC%" => SITE_WORK_JOB.$this->icon,
			"%LANGUAGE%" => $l_content
		);
	...
	public function dataGrid() {
		...
		foreach ($qrySel as $fetchRes) {
			...
			$iconCont = '<img src="'.SITE_WORK_JOB.$fetchRes["icon"].'" alt="No inage available"  style="width: 150px;height: auto;" />';
			$final_array = array(
	...
	public function contentSubmit($data, $files, $Permission) {
		...
		if($type=='edit' && !empty($_FILES['testiImage']['name']) && empty($_FILES['testiImage']['error'])) {
			$file_name = uploadFile($_FILES['testiImage'], DIR_WORK_JOB, SITE_UPD);
