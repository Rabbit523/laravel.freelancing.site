...
class ServiceCommision extends Home {
    ...
    public function dataGrid() 
    {
        ...
        // $whereCond = "";
        $chr = trim($chr);
        $whereCond .= " where commisionType = 'S' ";
        if (isset($chr) && $chr != '') {
    ...
    public function contentSubmit($data,$Permission){
        ...
        if($objPost->minPrice != ""  && $objPost->maxPrice!=""){
            ...
            $query=$this->db->pdoQuery("SELECT minPrice,maxPrice from tbl_commision where commisionType = 'S' and  id != $id and (($objPost->minPrice between minPrice and maxPrice) or ($objPost->maxPrice between minPrice and maxPrice))");
        ...
        if ($type == 'edit' && $id > 0) {
            ...
            if (in_array('add', $Permission)) {             
               ...
               add_admin_activity($activity_array);
               $response['status'] = true;
               $response['success'] = "Service Commission added successfully";
               echo json_encode($response);
               exit;
            } else {
                $response['error'] = "You don't have permission to add Service Commission";
                echo json_encode($response);
                exit;
            }
        }
