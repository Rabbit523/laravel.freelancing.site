...
class Job extends Home
{
    ...
    public function viewForm()
    {
        ...
        $content =
            ...
            $this->displayBox(array("label" => "Job Poster Name &nbsp;:", "value" => filtering($unm)))
        ;
        return $content;
    }
...
*/
        ...
        $fields = array(
            ...
            "%EST1%" => ($this->estimatedDuration == '5-10 Days') ? 'selected' : '',
            "%EST2%" => ($this->estimatedDuration == '10-15 Days') ? 'selected' : '',
            "%EST3%" => ($this->estimatedDuration == '15-30 Days') ? 'selected' : '',
            "%EST4%" => ($this->estimatedDuration == '1-3 Month') ? 'selected' : '',
            "%EST5%" => ($this->estimatedDuration == 'Upto 3 Month') ? 'selected' : '',
    ...
    public function dataGrid()
    {
        ...
        }else if (isset($filterSkill) && $filterSkill != ''){
             if ($whereCond) {
                $whereCond .= " AND ";
            } else {
                $whereCond .= " WHERE ";
        ...
        }else if (isset($filterJobStatus) && $filterJobStatus != ''){
             if ($whereCond) {
                $whereCond .= " AND ";
            } else {
                $whereCond .= " WHERE ";
        ...
        $qrySel = $this->db->pdoQuery("SELECT j.isApproved,j.isActive,j.isDelete,j.jobTitle,jc.category_name,j.budget,j.jobType,CONCAT(u.firstName,' ',u.lastName) As username,j.id As jobID FROM tbl_jobs As j
    ...
    public function getSubCategory(){
        ...
        $company_content = $company_content->compile();
        $qryCompany = $this->db->pdoQuery("SELECT * from tbl_subcategory WHERE isActive='y'")->results();
              foreach ($qryCompany as $key => $value) {
                    $fields = array(
