<?php
	...
	class MyPayPal  extends Home{
		...
		function SetExpressCheckout($products, $charges, $noshipping='1'){
			...
			$padata .= 	'&RETURNURL='.urlencode(ADM_PPL_RETURN_URL);
			$padata .=	'&CANCELURL='.urlencode(ADM_PPL_CANCEL_URL);
		...
		function DoExpressCheckoutPayment(){
			...
			if(!empty($_SESSION['ppl_products'])&&!empty($_SESSION['ppl_charges'])){
				...
				foreach($products as $p => $item){
					...
					$padata .=	'&L_PAYMENTREQUEST_0_AMT'.$p.'='.urlencode($item['ItemPrice']);
					$padata .=	'&L_PAYMENTREQUEST_0_QTY'.$p.'='. urlencode($item['ItemQty']);
					$padata .=	'&PAYMENTREQUEST_0_CUSTOM='. urlencode($item['PAYMENTREQUEST_0_CUSTOM']);
				}
				...
				if("SUCCESS" == strtoupper($httpParsedResponseAr["ACK"]) || "SUCCESSWITHWARNING" == strtoupper($httpParsedResponseAr["ACK"])){
					...
					if('Completed' == $httpParsedResponseAr["PAYMENTINFO_0_PAYMENTSTATUS"]){
						...
						include('../../units-sd/redeem_request-sd/return.php');
					...
					$msgType = $_SESSION["msgType"] = disMessage(array('type'=>'suc','var'=>'<div style="color:red">Error : '.urldecode($httpParsedResponseAr["L_LONGMESSAGE0"])));
				}
			}
			else{
