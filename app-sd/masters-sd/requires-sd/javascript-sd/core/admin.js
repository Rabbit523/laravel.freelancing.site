function displayMessagePart(isAjax) {
    if (isAjax == true) {
        urlPath = siteName + 'masters-sd/requires-sd/displayMessage-sd.php';
        $.ajax({
            type: "GET",
            url: urlPath,
...
function ajaxListingFunction(divID, module, action, id, value) {
    ...
    if (myConfirm == true) {
        $('#' + divID + '').html('<div style="margin:80px; text-align:center;"><div style="padding:18px;"><img src="' + siteName + 'frames-sd/images/loadingWait.gif" alt="" border="0" /><\/div><\/div>');

        urlPath = siteName + 'masters-sd/units-sd/' + module + '/ajax.' + module + '.php?action=' + action + '&id=' + id + '&value=' + value;
        $.ajax({
            type: "GET",
            url: urlPath,
            success: function (response) {
...
function search_process(){
    if ($(window).width() < 768) {
        $('.dataTables_filter input').not('.custom_datatable_search').closest('label').css('display','none');
        //$('.dataTables_filter [type=text]').not('.custom_datatable_search').closest('label').css('display','none');
        /*if($('.dataTables_filter .custom_datatable_search').length == 0){
            //$('.dataTables_filter').append('<label><input type="search" class="form-control input-inline custom_datatable_search" placeholder="Search" aria-controls="example123"></label>');
            $('.dataTables_filter .custom_datatable_search').val($('.dataTables_filter input').not('.custom_datatable_search').val());
        }else{
            $('.dataTables_filter .custom_datatable_search').closest('label').css('display','block');
        }*/
        $('.dataTables_filter .custom_datatable_search').closest('label').css('display','block');
    }else{
        //if($('.dataTables_filter .custom_datatable_search').length != 0){
            $('.dataTables_filter .custom_datatable_search').closest('label').css('display','none');
        //}
        $('.dataTables_filter input').not('.custom_datatable_search').closest('label').css('display','block');
    }
}

$(document).on('click','.ex_tabs_home a',function(e){
    var ids = $(this).attr('href'); 
    setTimeout(function(){ 
        if($(ids+' .dataTables_filter').find('.custom_datatable_search').length == 0){
            $(ids+' .dataTables_filter').find('label').addClass('temp_filter_class');
            console.log($(ids+'').find('.dataTables_filter'));
            $(ids+' .dataTables_filter').append('<label><input type="search" class="form-control input-inline custom_datatable_search" placeholder="Search" aria-controls="example123"></label>');
        }
    }, 150);
});
$(document).ready(function() {
    $('.dataTables_filter').each(function(){
        $(this).find('label').addClass('temp_filter_class');
        $(this).append('<label><input type="search" class="form-control input-inline custom_datatable_search" placeholder="Search" aria-controls="example123"></label>');
    });
    //search_process();
});

$(document).on('input','.custom_datatable_search',function(){

    if ($(window).width() < 768) {
        $('.dataTables_filter input').not('.custom_datatable_search').val($(this).val());
        $('.dataTables_filter input').not('.custom_datatable_search').trigger('input');
    }else{
        $('.dataTables_filter .custom_datatable_search').val($('.dataTables_filter input').not('.custom_datatable_search').val());
    }
});
