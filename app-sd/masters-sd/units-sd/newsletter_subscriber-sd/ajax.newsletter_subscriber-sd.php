...
if (isset($_REQUEST['ajaxvalidate']) && $_REQUEST['ajaxvalidate']) {
    ...
    if ($pageName != '' && $id != '') {
        if (getTotalRows("tbl_newsletter_subscriber", "pageName='" . $pageName . "' AND id != '" . $id . "' ", 'id') >= 1) {
            echo 'false';
            exit;
        } else {
            echo 'true';
    ...
    echo json_encode(array('type' => 'success', 'Newsletter Subscriber ' . ($value == 'a' ? 'activated ' : 'deactivated ') . 'successfully'));
    ...
    if ($affected_rows && $affected_rows > 0) {
        ...
        echo json_encode(array('type' => 'success', 'message' => "Newsletter Subscriber deleted successfully"));
        exit;
    } else {
        echo json_encode(array('type' => 'error', 'message' => "There seems to be an issue deleting Newsletter Subscriber"));
        exit;
    }
} else if ($action == "view") {
...
$mainObject = new NewsletterSubscriber($module, $id, NULL, $searchArray, $action);
