...
include(DIR_ADMIN_CLASS."abuse-keyword-sd.lib.php");
//echo "<pre>";print_r($_REQUEST);exit;
$module = 'abuse-keyword-sd';
chkPermission($module);
$Permission = chkModulePermission($module);
$table = 'tbl_abuse_keyword';
...
if (isset($_REQUEST['ajaxvalidate']) && $_REQUEST['ajaxvalidate']) {
    $keywordName = strtolower(filtering($_REQUEST['keywordName'], 'input'));
    $Id = filtering($_REQUEST['id'], 'input', 'int');
    if ($keywordName != '' && $Id != '') {
        if (getTotalRows("tbl_abuse_keyword", "LOWER(keywordName)='" . $keywordName . "' AND id != '" . $Id . "' ", 'id') >= 1) {
            echo 'false';
            exit;
        } else {
            echo 'true';
    ...
    echo json_encode(array('type' => 'success', 'Abuse Keyword ' . ($value == 'a' ? 'activated ' : 'deactivated ') . 'successfully'));
    ...
    if ($affected_rows && $affected_rows > 0) {
        ...
        echo json_encode(array('type' => 'success', 'message' => "Abuse Keyword deleted successfully"));
        exit;
    } else {
        echo json_encode(array('type' => 'error', 'message' => "There seems to be an issue deleting Keyword"));
        exit;
    }
} else if ($action == "view") {
...
$mainObject = new AbuseKeyword($module, $id, NULL, $searchArray, $action);
