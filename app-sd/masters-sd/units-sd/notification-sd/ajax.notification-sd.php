...
include(DIR_ADMIN_CLASS."notification-sd.lib.php");
...
if (isset($_REQUEST['ajaxvalidate']) && $_REQUEST['ajaxvalidate']) {
    ...
    if ($pageName != '' && $pId != '') {
        if (getTotalRows("tbl_notification", "message='" . $pageName . "' AND id != '" . $pId . "' ", 'id') >= 1) {
            echo 'false';
            exit;
        } else {
            echo 'true';
    ...
    echo json_encode(array('type' => 'success', 'Notification ' . ($value == 'a' ? 'activated ' : 'deactivated ') . 'successfully'));
    ...
    if ($affected_rows && $affected_rows > 0) {
        ...
        echo json_encode(array('type' => 'success', 'message' => "Notification deleted successfully"));
        exit;
    } else {
        echo json_encode(array('type' => 'error', 'message' => "There seems to be an issue deleting Notification"));
        exit;
    }
}else if ($action == "view") {
...
$mainObject = new Notification($module, $id, NULL, $searchArray, $action);
