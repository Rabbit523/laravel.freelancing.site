...
include(DIR_ADMIN_CLASS."payment_history-sd.lib.php");
//echo "<pre>";print_r($_REQUEST);exit;
$module = 'payment_history-sd';
chkPermission($module);
$Permission = chkModulePermission($module);
$table = 'tbl_commision';
$action = isset($_GET["action"]) ? trim($_GET["action"]) : (isset($_POST["action"]) ? trim($_POST["action"]) : 'datagrid');
$id = isset($_GET["id"]) ? trim($_GET["id"]) : (isset($_POST["id"]) ? trim($_POST["id"]) : 0);
$wallet_id = isset($_GET["wallet_id"]) ? trim($_GET["wallet_id"]) : (isset($_POST["wallet_id"]) ? trim($_POST["wallet_id"]) : 0);
...
/*if (isset($_REQUEST['ajaxvalidate']) && $_REQUEST['ajaxvalidate']) {
    ...
    if ($affected_rows && $affected_rows > 0) {
        ...
        echo json_encode(array('type' => 'success', 'message' => "Commission deleted successfully"));
        exit;
    } else {
        echo json_encode(array('type' => 'error', 'message' => "There seems to be an issue deleting Commission"));
        exit;
    }
} else if ($action == "view") {
    if(in_array('view', $Permission)) {
...
$mainObject = new PaymentHistory($module, $id,$wallet_id, NULL, $searchArray, $action);
