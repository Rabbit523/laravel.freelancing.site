...
if (isset($_REQUEST['ajaxvalidate']) && $_REQUEST['ajaxvalidate']) {
    $question = strtolower(filtering($_REQUEST['question'], 'input'));
    $id = filtering($_REQUEST['id'], 'input', 'int');
    if(empty($question) && empty($id)){
        echo 'true';
        exit;
    }
    $faqCategoryId = (isset($_REQUEST['faqCategoryId']) && $_REQUEST['faqCategoryId'] > 0)?filtering($_REQUEST['faqCategoryId'], 'input', 'int'):0;
    if ($question != '' && $id != '') {
        if (getTotalRows("tbl_faq", "LOWER(question)='" . $question . "' AND id != '" . $id . "' AND faqCategoryId ='".$faqCategoryId."' ", 'id') >= 1) {
            echo 'false';
            exit;
        } else {
            echo 'true';
    ...
    echo json_encode(array('type' => 'success', 'FAQ ' . ($value == 'a' ? 'activated ' : 'deactivated ') . 'successfully'));
    ...
    if ($affected_rows && $affected_rows > 0) {
        ...
        echo json_encode(array('type' => 'success', 'message' => "FAQ deleted successfully"));
        exit;
    } else {
        echo json_encode(array('type' => 'error', 'message' => "There seems to be an issue deleting FAQ"));
        exit;
    }
} else if ($action == "view") {
