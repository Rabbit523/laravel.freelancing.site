...
include(DIR_ADMIN_CLASS."niche-sd.lib.php");
//echo "<pre>";print_r($_REQUEST);exit;
$module = 'niche-sd';
chkPermission($module);
$Permission = chkModulePermission($module);
$table = 'tbl_listing_niche';
...
if (isset($_REQUEST['ajaxvalidate']) && $_REQUEST['ajaxvalidate']) {
    $nicheName = strtolower(filtering($_REQUEST['nicheName'], 'input'));
    ...
    if ($nicheName != '' && $id != '') {
        if (getTotalRows("tbl_listing_niche", "LOWER(nicheName)='" . $nicheName . "' AND id != '" . $id . "' AND parent_id ='".$parentId."' ", 'id') >= 1) {
            echo 'false';
            exit;
        } else {
            echo 'true';
    ...
    echo json_encode(array('type' => 'success', 'Listing Category ' . ($value == 'a' ? 'activated ' : 'deactivated ') . 'successfully'));
    ...
    if ($affected_rows && $affected_rows > 0) {
        ...
        echo json_encode(array('type' => 'success', 'message' => "Listing Category deleted successfully"));
        exit;
    } else {
        echo json_encode(array('type' => 'error', 'message' => "There seems to be an issue deleting Listing Category"));
        exit;
    }
} 
    ...
    if ($affected_rows && $affected_rows > 0) {
        ...
        echo json_encode(array('type' => 'success', 'message' => "Listing Category activated successfully"));
        exit;
    } else {
        echo json_encode(array('type' => 'error', 'message' => "There seems to be an issue activate Listing Category"));
        exit;
    }
} 
else if ($action == "view") {
...
$mainObject = new Niche($module, $id, NULL, $searchArray, $action);
