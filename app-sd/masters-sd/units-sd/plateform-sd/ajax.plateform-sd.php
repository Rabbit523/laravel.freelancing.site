...
include(DIR_ADMIN_CLASS."plateform-sd.lib.php");
//echo "<pre>";print_r($_REQUEST);exit;
$module = 'plateform-sd';
chkPermission($module);
$Permission = chkModulePermission($module);
$table = 'tbl_listing_platform';
...
if (isset($_REQUEST['ajaxvalidate']) && $_REQUEST['ajaxvalidate']) {
    $platformName = strtolower(filtering($_REQUEST['platformName'], 'input'));
    $id = filtering($_REQUEST['id'], 'input', 'int');
    if ($platformName != '' && $id != '') {
        if (getTotalRows("tbl_listing_platform", "LOWER(platformName)='" . $platformName . "' AND id != '" . $id . "' ", 'id') >= 1) {
            echo 'false';
            exit;
        } else {
            echo 'true';
    ...
    echo json_encode(array('type' => 'success', 'Listing Platform ' . ($value == 'a' ? 'activated ' : 'deactivated ') . 'successfully'));
    ...
    if ($affected_rows && $affected_rows > 0) {
        ...
        echo json_encode(array('type' => 'success', 'message' => "Listing Platform deleted successfully"));
        exit;
    } else {
        echo json_encode(array('type' => 'error', 'message' => "There seems to be an issue deleting Listing Platform"));
        exit;
    }
} 
    ...
    if ($affected_rows && $affected_rows > 0) {
        ...
        echo json_encode(array('type' => 'success', 'message' => "Listing Platform activated successfully"));
        exit;
    } else {
        echo json_encode(array('type' => 'error', 'message' => "There seems to be an issue activate Listing Platform"));
        exit;
    }
}
else if ($action == "view") {
...
$mainObject = new Plateform($module, $id, NULL, $searchArray, $action);
