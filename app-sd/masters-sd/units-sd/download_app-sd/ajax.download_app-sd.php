<?php
	...
	if ($action == "updateStatus" && in_array('status', $Permission)) {
		...
		if (isset($id) && isset($language_id)) {
			...
			if (empty($language_id)) {
				$selectArr[]= 'tag_line'.' as tag_line ';
				$selectArr[]= 'title'.' as title ';
				$selectArr[]= 'content'.' as content ';
			} else {
				$titleFld = "tag_line_".$language_id.' as tag_line ';
			...
			$qrySel = $db->pdoQuery("select ".$selectArr." from tbl_download_app where id=? ",array($id))->result();
	...
	$mainObject = new DownloadApp($module, $id, NULL, $searchArray, $action);
