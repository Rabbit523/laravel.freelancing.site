<?php
	...
	include(DIR_ADMIN_CLASS."way_to_work_service-sd.lib.php");
	
	$module = "way_to_work_service-sd";
	chkPermission($module);
	$Permission = chkModulePermission($module);
	$table = "tbl_way_to_work_service";
	...
	if ($action == "updateStatus" && in_array('status', $Permission)) {
		...
		if (isset($id) && isset($language_id)) {
			...
			$qrySel = $db->pdoQuery("select ".$selectArr." from tbl_way_to_work_service where id=? ",array($id))->result();
	...
	$mainObject = new WayToWorkService($module, $id, NULL, $searchArray, $action);
