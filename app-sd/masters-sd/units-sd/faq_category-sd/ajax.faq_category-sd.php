...
include(DIR_ADMIN_CLASS."faq_category-sd.lib.php");
//echo "<pre>";print_r($_REQUEST);exit;
$module = 'faq_category-sd';
...
if (isset($_REQUEST['ajaxvalidate']) && $_REQUEST['ajaxvalidate']) {
    ...
    if ($categoryName != '' && $id != '') {
        if (getTotalRows("tbl_faq_category", "LOWER(categoryName)='".$categoryName."' AND  id != '" . $id . "' ", 'id') >= 1) {
            echo 'false';
            exit;
        } else {
            echo 'true';
    ...
    echo json_encode(array('type' => 'success', 'FAQ Category ' . ($value == 'a' ? 'activated ' : 'deactivated ') . 'successfully'));
    ...
    if ($affected_rows && $affected_rows > 0) {
        ...
        echo json_encode(array('type' => 'success', 'message' => "FAQ Category deleted successfully"));
        exit;
    } else {
        echo json_encode(array('type' => 'error', 'message' => "There seems to be an issue deleting FAQ Category"));
        exit;
    }
} else if ($action == "view") {
...
$mainObject = new FaqCategory($module, $id, NULL, $searchArray, $action);
