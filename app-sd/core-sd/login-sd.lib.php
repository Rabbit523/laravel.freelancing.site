...
class Login {
	...
	public function getPageContent() {
		...
		else
		{
	        ...
	        $error_class = !empty($_SESSION["login_error"])?"":"hide";
        	
	        return str_replace(array('%EAMIL_COOKIE%','%PASSWORD_COOKIE%','%COOKIE_CHECKED%',"%ERROR_CLASS%"), array($email_cookie,$password_cookie,$remember_cookie,$error_class), $html);
		}        
	}
	public function loginSubmit($data)
	{
		...
		if (isset($email) && isset($password)) {
			...
			if (!empty($selQuery)) {
				...
				$_SESSION["login_error"] = PASSWORD_DID_NOT_MATCH;
				if($userType=="F"){
					redirectPage(SITE_URL."f/profile");
				}else{
					redirectPage(SITE_URL."profile");							
