...
class FreelancerMyBids {
	function __construct($module = "", $id = 0, $token = "") {
  	...
  	{
  		$sub_content = new MainTemplater(DIR_TMPL . $this->module . "/freelancer_mybids-sd.skd");
        $sub_content = $sub_content->compile();

        $where = "jb.userId ='".$this->sessUserId."' and j.id IS NOT NULL";
        ...
        $data = str_replace(array("%SUB_HEADER_CONTENT%","%LOOP_DATA%","%LOAD_CLASS%"), array(subHeaderContent("my-bids"),$this->job_data(1),$load_class), $sub_content);
        return $data;
    }
    ...
    public function job_data($pageNo)
	{
	    ...
	    $query = $this->db->pdoQuery("select j.*,jb.isHired,c.".l_values('category_name')." as category_name,jb.budget as bid_amt,s.".l_values('subcategory_name')." as subcategory_name,u.firstName,u.lastName,u.location,j.jobStatus,u.userSlug, u.id as user_id from tbl_job_bids As jb
	    	LEFT JOIN tbl_jobs As j ON j.id = jb.jobId
	    	LEFT JOIN tbl_category As c ON c.id = j.jobCategory
	        LEFT JOIN tbl_subcategory As s ON s.id = j.jobSubCategory
	        LEFT JOIN tbl_users As u ON u.id = j.posterId
	        where ".$where." ORDER BY jb.id DESC LIMIT ".$start_from.",".$num_rec_per_page)->results();
	    ...
	    foreach ($query as $value) {
	        ...
	        $array = array(
	            ...
	            "%BID_AMOUNT%" => $value['bid_amt']."<span>".CURRENCY_SYMBOL."</span>",
	            ...
	            "%BID_STATUS%" =>getBidStatus($value['isHired']),
