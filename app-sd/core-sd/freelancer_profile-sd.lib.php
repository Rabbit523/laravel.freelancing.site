...
class FreelancerProfile {
	function __construct($module = "", $id = 0, $token = "") {
  	...
  	{
      global $sessUserType;
  		$sub_content = new MainTemplater(DIR_TMPL . $this->module . "/freelancer_profile-sd.skd");
      $sub_content = $sub_content->compile();
          ...
          $profileUrl = SITE_URL."f/profile/".$query['userSlug'];
            ...
            "%SUB_HEADER_CONTENT%" => subHeaderContent("profile"),
    ...
    public function education()
    {
        ...
        foreach ($query as $value) {
            ...
            $array = array(
                ...
                "%START_YR%" => !empty($value['start_year'])?($value['start_year']):"",
                "%END_YR%" => !empty($value['end_year'])?($value['end_year']):"",
    ...
    public function data_add($data,$type)
    {
      ...
      if($type=="profile" || $type=="location" || $type=="proTitle" ){
        $email = getTableValue("tbl_users","email",array("id"=>$this->sessUserId));
        $this->db->update("tbl_users",$array,array("email"=>$email));
      }else{
