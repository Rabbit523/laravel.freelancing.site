...
include(DIR_CLASS."freelancer_account_setting-sd.lib.php");
...
{
	if(!empty($_GET['email'])){
		$user_deatils = $db->pdoQuery("SELECT * FROM `tbl_users` WHERE MD5(paypal_email_new) = ? ",[$_GET['email']])->result();
		//echo $_GET['email'];
		//exit;
		if(!empty($user_deatils)){
			...
			redirectPage(SITE_URL.'f/account-setting','refresh');
		}
	}
...
if($action == "verify_paypalId")
{
	...
	/*$msgType =$_SESSION["msgType"]=disMessage(array('type' => 'err', 'var' => 'Password Id  is not valid'));
	echo "false";
	exit;*/
	...
	if(!empty($paypal_email)){
		...
		if($data->responseEnvelope->ack == "Success")
		{
			...
			$link = SITE_URL.'AjaxAccountSetting?action=varify_new_paypal_email&email='.md5($_POST['paypal_email']);
			...
			$msgType =$_SESSION["msgType"]=disMessage(array('type' => 'err', 'var' => PAYPAL_EMAIL_IS_NOT_VALID));
			echo "false";
			exit;
		}
