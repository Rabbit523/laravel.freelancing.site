...
class PdoWrapper extends PDO {
    ...
    public function set_log($msg){
        $fp = fopen(__DIR__.'/data/error_log', 'a');
        fwrite($fp, "Executed Query-> ".$this->interpolateQuery()."\n");
        fwrite($fp, $msg."\n\n");
        fclose($fp);
    }
    /*changes made by JJ*/
    ...
    public function pdoQuery( $sSql = '', $aBindWhereParam = array() ) {
        ...
        if ( !empty( $sSql ) && count( $aBindWhereParam ) <= 0 ) {
            ...
            try {
                ...
                /*changes made by JJ*/
                $this->set_log($e->getMessage() . ': ' . __LINE__.' @ '.date('Y-m-d H:i:s'));
                $site_path = SITE_URL."wrong-turn/";
